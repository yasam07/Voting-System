...
{{--Page Header--}}
    <div >
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Location Management  / Districts</h2>
            <p class="text-sm text-gray-600">
                Locations information of Districts
            </p>
        </div>
    </div>
    ...
    <div class="flex justify-between">
        ...
        <x-partials.button class="mt-6" href="{{ route('admin.locations.districts.create') }}" variant="primary">Create district</x-partials.button>
    </div>
    <div class="bg-white shadow rounded-lg overflow-x-auto mb-6">
        <table class="w-full border-collapse text-sm">
            ...
            <tbody class="border-b">
                ...
                <tr class="border-b">
                    <td class="px-4 py-3 w-5 text-center">{{ $loop->iteration }}</td>
                    <td class="px-4 py-3 w-30 text-center">{{ $district->name }}</td>
                    <td class="px-4 py-3 w-30 text-center">{{ $district->province->name }}</td>
                    <td class="px-4 py-3 w-30 text-center">
                        <span class="px-3 py-1 rounded-full text-sm
                            {{ $district->status ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold' }}">
                    ...
                    <td class="px-6 py-2 w-5">
                        <div class="flex justify-center gap-3 items-center">
                            ...
                            <div>
                                <form action="{{ route('admin.locations.districts.destroy', $district->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this voter?');" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit">
